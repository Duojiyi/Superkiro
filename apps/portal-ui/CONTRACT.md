...
`signature` 不表示清单自身已签名或 Mac 已公证。Mac 版已完成真机接管验收（2026-09-26），不再标注测试版；安装包仍未经 Apple 签名与公证，使用文档保留首次打开的放行说明。

页面不合成下载地址、不提供默认版本。清单不可用、格式错误或条目校验失败时禁用相应下载；重复平台架构也禁用。下载区只放下载按钮（Windows 一个，Mac 按芯片两个），不展示版本、大小、系统要求、签名状态或校验值；只有清单无法加载或有平台缺失时才显示一行提示与重试。浏览器不做安装包验签；客户端的自动更新另按 `deploy/UPDATE-RELEASE.md` 用离线密钥签名的清单条目校验。
