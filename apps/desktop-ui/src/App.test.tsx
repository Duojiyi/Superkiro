...
describe('host lock and failed restore guidance',()=>{
 ...
 it('a restore stopped by a settings.json syntax error says which line to fix and nothing else',async()=>{setup(true,true);HTMLDialogElement.prototype.showModal=function(){this.open=true;};const original=invoke.getMockImplementation()!;invoke.mockImplementation(async(command,payload)=>payload.path==='/api/restore'?Promise.reject({...supportError('SK-RESTORE-003'),line:12,column:5}):original(command,payload));render(<App/>);await screen.findByRole('heading',{name:'本机配置待恢复'});fireEvent.click(screen.getByRole('button',{name:'还原 Kiro 配置'}));fireEvent.submit(document.querySelector('dialog form')!);await screen.findByRole('heading',{name:'恢复未完成'});const page=document.querySelector('main')!;expect(page.querySelector('.subtitle')!.textContent).toContain('没有修改任何文件');expect(screen.getByRole('region',{name:'错误反馈'}).textContent).toContain('第 12 行第 5 列');expect(page.textContent).not.toContain('文件占用');expect(screen.queryByRole('button',{name:'下载 Kiro ↗'})).toBeNull();expect(screen.getByRole('button',{name:'重新恢复'})).toBeTruthy();});
...
describe('a newly installed version confirming itself',()=>{
 it('confirms once a page with the host status has rendered, and only once',async()=>{setup(true);render(<App/>);await screen.findByRole('heading',{name:'卡密登录'});await waitFor(()=>expect(invoke).toHaveBeenCalledWith('native',{method:'update_confirm',args:[]}));fireEvent.click(screen.getByRole('button',{name:'登录 →'}));await act(async()=>{await Promise.resolve();});expect(invoke.mock.calls.filter(([c,p])=>c==='native'&&p.method==='update_confirm')).toHaveLength(1);});
 it('a first status that fails is tried again, routes the page, confirms and clears its error',async()=>{setup(true);const original=invoke.getMockImplementation()!;let failures=0;invoke.mockImplementation(async(command,payload)=>{if(payload.path==='/api/status'&&failures++<1)throw supportError('SK-LOCAL-001');const result=await original(command,payload);return payload.path==='/api/status'?{...result,authenticated:true,has_snapshot:true}:result;});render(<App/>);await screen.findByRole('region',{name:'错误反馈'});expect(invoke).not.toHaveBeenCalledWith('native',{method:'update_confirm',args:[]});await screen.findByRole('button',{name:'打开 Kiro ↗'},{timeout:5000});await waitFor(()=>expect(invoke).toHaveBeenCalledWith('native',{method:'update_confirm',args:[]}));expect(screen.queryByRole('region',{name:'错误反馈'})).toBeNull();});
});
