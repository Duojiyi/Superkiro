...
export function App() {
  ...
  const sessionStatus=useRef<Status>({}), verificationOnly=useRef(false);
  ...
  function applyStatus(s:Status){sessionStatus.current=s;setStatus(s);}
  ...
  function resetSessionData(){invalidateMemory();sessionGeneration.current++;loadingUsage.current=false;setUsage(null);setUsageState('idle');setAuth(null);setBalanceTime('');verificationOnly.current=false;}
  ...
  }catch{if(generation===sessionGeneration.current)setUsageState('error');}finally{if(generation===sessionGeneration.current)loadingUsage.current=false;} }
  useEffect(()=>{
    if(!uncertain)return;
  ...
  function ask(action:Intent){if(lock.current)return;if(uncertain){setNotice('上次写入结果未确认，正在查询宿主操作状态，确认结束前暂不允许重复修改配置。');setPage('doctor');return;}
    ...
    setModal({action,title:titles[action],label:action==='activate'?'启用连接':action==='trim'?'整理工作集':action==='exit'?'还原并退出':'确认并继续',detail:action==='trim'?'仅请求系统整理实际 Kiro 进程工作集，不终止正在编辑的进程。':action==='activate'?'将备份原始配置并配置连接，可能需要重启 Kiro。请先保存文件。配置完成不代表模型对话已验证。':'请先保存文件。确认后将自动关闭 Kiro；未能正常退出时会结束进程，未保存内容可能丢失。仅还原 Superkiro 修改的配置，失败保留备份。'});
  }
  ...
  async function execute(action:Intent,unbindCard:string){setModal(null);await perform(async()=>{
    ...
    try{if(!(action==='exit'&&status.authenticated===false&&status.has_snapshot===false&&status.recovery_pending!==true))await mutate(action==='unbind'?'/api/unbind':'/api/restore',action==='unbind'?{card_key:unbindCard,close_kiro_confirmed:true}:{close_kiro_confirmed:true});
  ...
  return <div className={`shell ${page==='login'?'login-shell':''}`} aria-busy={!!pendingLabel} onMouseDown={event=>{
    ...
    {page==='restore-failed'&&<section><h1>恢复未完成</h1><p className="subtitle">请先检查恢复结果，再退出程序。</p><div className="panel alert spaced"><h2>无法确认原始配置已恢复</h2><p>请检查文件占用或权限后重试。前端不会删除备份。</p></div><button className="primary full" disabled={busy||uncertain} onClick={()=>ask('restore')}>重新恢复</button><button className="full" onClick={()=>navigate('doctor')}>查看连接诊断</button></section>}
