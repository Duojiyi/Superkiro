...
import { useUpdater, UpdateScreen } from './Updater';
...
export function App() {
  ...
  // A mandatory update waits for anything in progress, then installs and restarts the client.
  const updater=useUpdater(busy||!!modal||page==='connecting',version=>setNotice(`Superkiro 已更新到 ${version}。`));
  ...
  return <div className={`shell ${page==='login'?'login-shell':''}`} aria-busy={!!pendingLabel} onMouseDown={event=>{
    ...
    <header><strong className="brand">Superkiro</strong><div className="window-drag"/>{pendingLabel&&<span className="pending-indicator" role="status"><span className="sr-only">{pendingLabel}</span></span>}<button className="website-button" aria-label="打开官网" title="打开官网" onClick={()=>void perform(async()=>{await native('open_external',['https://kiro.rent/']);})}>官网 ↗</button>{updater.offer&&<button className="website-button update-offer" title={updater.postponed&&updater.mandatory?`必需更新未完成 · 新版本 ${updater.offer.version}`:`新版本 ${updater.offer.version} 可用`} onClick={()=>updater.start()}>{updater.postponed&&updater.mandatory?'继续更新':`新版本 ${updater.offer.version}`} ↑</button>}<Announcements blocked={busy||uncertain||!!modal||!hasSession||page==='login'||updater.screen}/><div className="window-controls"><button aria-label="最小化" onClick={()=>void perform(async()=>{await native('minimize');})}>−</button><button aria-label="关闭窗口" disabled={busy&&!status.tray_available} onClick={()=>closeBehavior==='exit'?ask('exit'):closeBehavior==='minimize'?void native('minimize').catch(e=>captureError(e)):status.tray_available?void native('close').catch(e=>captureError(e)):ask('exit')}>×</button></div></header>
    {page!=='login'&&hasSession&&!updater.screen&&<nav aria-label="主导航">{tabs.map(([id,label,icon])=><button key={id} disabled={busy&&page==='connecting'} aria-current={(page===id||id==='overview'&&['connecting','restore-failed'].includes(page))?'page':undefined} onClick={()=>navigate(id)}><span aria-hidden="true">{icon}</span>{label}</button>)}</nav>}
    ...
    {updater.screen?<main className="page-update"><UpdateScreen updater={updater} blocked={busy||!!modal||page==='connecting'} openDownloads={()=>void perform(async()=>{await native('open_external',['https://kiro.rent/#downloads']);})} restore={status.has_snapshot===true||status.recovery_pending===true?()=>ask('restore'):null}/></main>:<main key={page} className={`page-${page}`}>
