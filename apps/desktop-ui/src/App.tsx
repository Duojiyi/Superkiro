...
export function App() {
  ...
  async function execute(action:Intent,unbindCard:string,force=false){setModal(null);if(remoteUnbound&&['activate','unbind'].includes(action)){ask(action);return;}if(uncertain&&!(remoteUncertain&&action==='restore')){ask(action);return;}await perform(async()=>{setRetry(null);
    ...
    }catch(e){if(remoteUncertain&&action==='restore'&&!mutation.current){mutation.current=true;setUncertain(true);setRemoteUncertain(true);}if(!mutation.current){setRetry({action,card:unbindCard,code:toClientError(e).code});setPage('restore-failed');}throw e;}
  });}
  ...
  return <div className={`shell ${page==='login'?'login-shell':''}`} aria-busy={!!pendingLabel} onMouseDown={event=>{
    ...
    {page==='restore-failed'&&<section><h1>恢复未完成</h1><p className="subtitle">{retry?.code==='SK-CONNECT-005'?'Kiro 仍未关闭，可能正在询问是否保存。请回到 Kiro 处理提示，或在 Kiro 中选择「文件 > 退出」（默认会保留未保存的内容），然后重试。':retry?.code==='SK-CONNECT-006'?'Kiro 在你的另一个 Windows 会话中仍在运行。请到那个会话中关闭 Kiro，然后重试。':'请检查文件占用或权限后重试，确认恢复前不要退出。'}</p><div className="panel alert spaced"><p>保留本机快照和扩展备份，不要删除或编辑，不要公开上传，暂停升级或重装 Kiro。</p><p>仍无法恢复时，请复制反馈信息，通过官网支持渠道提交错误码、反馈编号、客户端与 Kiro 版本及失败时间。</p></div><button className="primary full" disabled={busy||uncertain} onClick={()=>retry?void execute(retry.action,retry.card):ask('restore')}>重新恢复</button>{retry?.code==='SK-CONNECT-005'&&<button className="full" disabled={busy||uncertain} onClick={()=>setModal({action:retry.action,force:true,title:'强制关闭 Kiro？',label:'强制关闭并继续',detail:'Kiro 仍打开着，可能正在询问是否保存。强制关闭会直接结束 Kiro，未保存的内容将丢失且无法恢复。如需保留，请先回到 Kiro 保存，或在 Kiro 中选择「文件 > 退出」。'})}>强制关闭 Kiro 并继续</button>}</section>}
