...
const Editor = load('CommercialEditor.tsx', {'./tokens': load('tokens.ts'), './pricing': pricing, './api': {adminApi: {}}, './format': display, './status': load('status.ts'),
  // Dialogs and messages only run from event handlers; the render tree just names the components.
  './components/confirm': {confirmAction: async () => true}, './components/toast': {toast: {success() {}, info() {}, error() {}}},
  './components/ui': {InfoTip: 'InfoTip', StatusBadge: 'StatusBadge'}, react: {
...
assert(text(tree).includes('采购价（用于成本估算）'), 'procurement prices are labelled as cost estimates, not customer prices');
...
setup(); states[7].margin_multiplier = ''; find(render(), 'button', '加入价格草稿').props.onClick(); assert(states[4].includes('倍率需大于 0、不超过 1000'));
...
assert(text(history).includes(display.formatFullDateTime(version.effective_from_secs)));
