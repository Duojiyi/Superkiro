...
(async()=>{
 ...
 try {
  ...
  assert.equal(requestsBefore.length,0,'request content is read only when a content tab is opened');
  await page.getByRole('tab',{name:'请求内容'}).click();await drawer.getByText('那退出登录呢？（本地测试请求 #0）',{exact:true}).waitFor();
  ...
  await page.getByRole('tab',{name:'模型回复'}).click();await drawer.getByText('本地测试回复 #0',{exact:false}).waitFor();
  ...
  await page.getByRole('button',{name:'详情',exact:true}).first().click();await page.getByRole('tab',{name:'模型回复'}).click();
