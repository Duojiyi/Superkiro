...
module.exports = function fixtureApi() {
  ...
  return {writes, expire() {authenticated=false;}, async handle(req,res) {
    ...
    if(endpoint==='me' || (endpoint==='session' && req.method==='GET')) return reply({success:true,role:'admin',csrfToken:'fixture-csrf',expiresAt:Math.floor(Date.now()/1000)+900,twoFactorEnabled:false,totpRequired:false});
