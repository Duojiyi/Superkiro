...
export default function ProviderKeyEditor({ selectedKey, onSaved }: {selectedKey?: Record<string, unknown>; onSaved?: () => void}) {
  ...
  const [message, setMessage] = useState('发现仅返回候选模型，不证明可调用，不会自动开放给用户。');
  const act = async (action: 'save' | 'discover') => {
    ...
    try {
      ...
      } else { setSecret(''); onSaved?.(); setMessage('已保存 Key 权限。请刷新后台查看；到模型映射页显式发布用户目录。'); }
  ...
  const createProvider = async () => {
    ...
    setBusy(true);
    try {
      await adminApi.importProvider({providers: [{id: provider.trim(), name: provider.trim(), api_type: format, base_url: baseUrl.trim(), api_key: secret, models: models.split(/\r?\n/).map(s=>s.trim()).filter(Boolean)}]});
      setKeyId(provider.trim() + '-key-1'); setSecret(''); onSaved?.();
