...
export default function CommercialEditor({ kind }: {kind: 'groups' | 'models'}) {
  ...
  const stagePrice = () => {
    try {
      ...
      const versions = [...(parsedDraft.versions || []).filter(v => v.id !== priceDraft.id), Object.fromEntries(Object.entries({...priceDraft, ...values}).filter(([key]) => key !== 'source_id'))];
  ...
  try { parsedDraft = JSON.parse(draft); } catch { /* Keep invalid advanced edits visible for correction. */ }
  const rows = parsedDraft[kind] || [];
  ...
  const publish = async () => {
    ...
    try {
      ...
      setConfig(result.config); setReason('');
      setDraft(JSON.stringify(kind === 'groups' ? {groups: result.config.groups} : {models: result.config.models, rate_cards: result.config.rate_cards, versions: []}, null, 2));
