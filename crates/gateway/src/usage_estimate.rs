...
/// Size of `text` in quarter-token units. ASCII is about four characters per token;
/// CJK ideographs, kana, hangul and fullwidth forms about one token per character;
...
/// Estimate the input tokens of a request from its JSON. Image payloads count at a
/// fixed cost and their bytes are skipped; everything else, including tool schemas,
/// tool results and editor state, counts as text.
